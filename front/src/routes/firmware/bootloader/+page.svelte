<script lang="ts">
	...
	async function uploadFirmware() {
		...
		const result = await uploadWithProgress('/api/spiffs/STM32U5G9ZJTXQ_OSPI_Bootloader.bin', file, {
			maxSize: UPLOAD_LIMITS.SPIFFS,
			timeout: 360000, // 6 minutes
			onProgress: (percent) => {
				uploadProgress = percent;
				uploadMessage = `Uploading bootloader file... ${Math.floor(percent)}%`;
			},
			context: 'Bootloader upload'
		});
