...
static esp_err_t rest_common_get_handler(httpd_req_t *req)
{
    ...
    {
        strlcat(filepath, "/index.html.gz", sizeof(filepath));
    }
    else
    {
        strlcat(filepath, req->uri, sizeof(filepath));
        char gz_filepath[FILE_PATH_MAX + 4]; // Add 4 for ".gz" and null terminator
        snprintf(gz_filepath, sizeof(gz_filepath), "%s.gz", filepath);

        ESP_LOGI(TAG, "Checking for gzipped file using open: %s", gz_filepath);
        int fd = open(gz_filepath, O_RDONLY);
        if (fd >= 0)
        {
            close(fd);
            strlcpy(filepath, gz_filepath, sizeof(filepath));
        }
    }
    ...
    ssize_t read_bytes;
    while ((read_bytes = read(fd, chunk, SCRATCH_BUFSIZE)) > 0)
    {
